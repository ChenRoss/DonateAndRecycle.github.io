...
async function analyzePhotoWithAPI(photoData, apiKey) {
    ...
    try {
        ...
        if (analysis.includes("不可回收")) {
            recyclingStatus = "<span style='font-size: 26px;'>❌</span> 此物品不符合回收標準。";
        } else if (analysis.includes("無法分析") || analysis.includes("無法辨識")) {
            recyclingStatus = "<span style='font-size: 26px;'>❌</span> 可能含有人物或著作商標，請重新拍攝。";
        } else {
            recyclingStatus = "<span style='font-size: 26px;'>✅</span> 此物品符合回收標準。";
        }
